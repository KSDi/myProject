...
select * from product;
select * from cart;
select * from orders where regdate = sysdate order by u_id ASC;
select * from orders where status ='배송대기';
update orders set status='배송중' where addr like '서울%';
select sysdate from dual;
select * from users;
alter table users add (regdate date);
update users set regdate = sysdate;
select * from users where trunc(regdate) = trunc(sysdate);
select * from orders;